...
contract TycheStakeTemple is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    function _updateReward(address _user) internal {
        ...
        if (block.number > lastRewardBlock && stakedSupply != 0) {
            ...
            adjustedTokenPerShare = accTokenPerShare.add(_rewards.mul(PRECISION_FACTOR).div(stakedSupply));
        }
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 _rewards = multiplier.mul(rewardPerBlock);
        accTokenPerShare = accTokenPerShare.add(_rewards.mul(PRECISION_FACTOR).div(stakedSupply));
        lastRewardBlock = block.number;
    }
